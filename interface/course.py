...
def showCourseDetailsInterface():
    window = tkinter.Tk()
    ...
    def validate_course_id_input(event):
        if event.char.isalpha():
            course_id_entry.delete(len(course_id_entry.get())-1,tkinter.END)
    ...
    def submitButtonOnClick():
        ...
        entered_year = year_combobox.get()
        entered_teacher_id = teacher_combobox.get()
        ...
        if not entered_course_id or not entered_name or not entered_division or not entered_year or not entered_teacher_id:
            ...
            db.insertCourseDetails(entered_course_id, entered_name, entered_year, entered_division, entered_teacher_id)
            messagebox.showinfo(title="Database Info", message="Course details stored successfully.")
            ...
            teacher_combobox.set(teacherList[0])
    ...
    teacher_combobox = ttk.Combobox(frame, values=teacherList, font=("Arial", 16))
    ...
    teacher_combobox.grid(row=5, column=1, pady=20)
